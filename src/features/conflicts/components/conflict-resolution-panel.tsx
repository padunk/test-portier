          ...
          <li key={conflict.id} className="list-row list-row--conflict">
            ...
            <div className="decision-grid">
              <div style={{ display: "flex", flexDirection: "column" }}>
                <label htmlFor={`local-${conflict.id}`}>Keep local</label>
                <button
                  id={`local-${conflict.id}`}
                  aria-label={`Keep local value for ${formatFieldName(conflict.fieldName)}: ${formatValue(conflict.localValue)}`}
                ...
                >
                  <strong>{formatValue(conflict.localValue)}</strong>
                </button>
              </div>

              <div style={{ display: "flex", flexDirection: "column" }}>
                <label htmlFor={`external-${conflict.id}`}>Use external</label>
                <button
                  id={`external-${conflict.id}`}
                  aria-label={`Use external value for ${formatFieldName(conflict.fieldName)}: ${formatValue(conflict.externalValue)}`}
                ...
                >
                  <strong>{formatValue(conflict.externalValue)}</strong>
                </button>
              </div>
            </div>
